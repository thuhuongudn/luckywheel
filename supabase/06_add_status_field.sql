-- ============================================================================
-- MIGRATION: Add status field to lucky_wheel_spins
-- Version: 1.1
-- Date: 2025-10-17
...
BEGIN
    ...
    ELSE
        RAISE NOTICE 'Column status already exists in lucky_wheel_spins';
    END IF;
END $$;
...
-- Create index for status + campaign queries (admin dashboard)
CREATE INDEX IF NOT EXISTS idx_spins_campaign_status
  ON lucky_wheel_spins(campaign_id, status, created_at DESC);

-- ============================================================================
-- FUNCTION: Auto-update status based on expires_at
-- Purpose: Automatically set status to 'expired' when coupon expires
-- ============================================================================
...
BEGIN
  ...
  WHERE status = 'active'
    AND expires_at < NOW()
    AND status != 'used';  -- Don't override 'used' status
...
-- ============================================================================
-- TRIGGER: Auto-set expired status on SELECT (for real-time status)
-- Note: This is a simplified approach. For production, consider a cron job
-- ============================================================================
...
-- ============================================================================
-- UPDATE STATISTICS FUNCTION: Include status counts
-- ============================================================================
...
-- ============================================================================
-- INITIAL DATA: Set default status for existing records
-- ============================================================================
