...
/**
 * Format timestamp for Haravan API
 * Keep original UTC time - Haravan will handle timezone conversion
 * @param {Date|string} date
 * @returns {string} ISO 8601 string (UTC)
 */
function formatHaravanTimestamp(date) {
  const d = new Date(date);
  // Return ISO string as-is (UTC) - Haravan handles timezone
...
async function createDiscount({ code, value, starts_at, ends_at, campaignId }) {
  try {
    ...
    // Format timestamps for Haravan (keep UTC, let Haravan handle timezone)
    const startsAtFormatted = formatHaravanTimestamp(starts_at);
    const endsAtFormatted = formatHaravanTimestamp(ends_at);
    ...
    const body = {
      discount: {
        ...
        // Time range (UTC - Haravan handles timezone)
