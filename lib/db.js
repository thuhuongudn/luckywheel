...
async function saveSpin(spinData) {
  ...
  try {
    ...
    if (error && error.code === '42703') {
      console.warn('⚠️  [DB] phone_plain column missing (42703), falling back to legacy schema');
      const { phone_plain, ...legacyPayload } = basePayload;
      ...
      console.log('📊 [DB] Legacy insert result:', {
        hasData: !!data,
        hasError: !!error,
        errorCode: error?.code,
        errorMessage: error?.message
